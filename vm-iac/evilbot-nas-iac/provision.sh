...
scp -J "$JUMP" \
  ...
  "admin@$VM_IP:/tmp/" 2>/dev/null || true

ssh -o StrictHostKeyChecking=accept-new -J "$JUMP" "admin@$VM_IP" bash << 'ENDSSH'
set -euo pipefail
...
sudo groupadd -f user2group

echo "--- Users ---"
for u in mediaadmin mediauser user2; do
...
sudo usermod -aG user2group user2 2>/dev/null || true
...
[media_upload]
   ...
   browseable = no
   read only = no
   valid users = mediauser mediaadmin admin
   force group = mediagroup
   create mask = 0664
   directory mask = 2775
   inherit permissions = yes
...
[private_training]
   path = /tank/media/private/training
   browseable = yes
   read only = no
   valid users = user2 mediaadmin admin
   force group = user2group
   create mask = 0664
   directory mask = 2770
   inherit permissions = yes
